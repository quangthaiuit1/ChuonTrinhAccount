...
public class AccountService implements ImplAccount {
	@Inject
	...
	public Account create(Account account) {
		PrivateConfig cf = new PrivateConfig();
		em.persist(cf);
		cf = em.merge(cf);
		account.setPrivateConfig(cf);
		em.persist(account);
	...
	public Set<Program> findProgramByAccount(Account account) {
		String sql = "SELECT p.id, p.name,p.fullname ,p.uRL,p.serveraddress,p.serveraddressPublic,p.description FROM program as p "
		...
		for (Object[] values : returns) {
			...
			final String url = values[3] + "";
			final String serveraddress = values[4] + "";
			final String serveraddressPublic = values[5] + "";
			final String description = values[6] + "";
			Program pro = new Program();
			pro.setId(id);
			pro.setName(name);
			pro.setFullname(fullname);
			pro.setuRL(url);
			pro.setServeraddress(serveraddress);
			pro.setServeraddressPublic(serveraddressPublic);
	...
	public List<Account> findByDepartment(Department department) {
		...
		CriteriaQuery<Account> cq = cb.createQuery(Account.class);
		Root<Account> root = cq.from(Account.class);
		cq.select(root).where(cb.equal(root.get("member").get("department"), department),cb.equal(root.get("disable"), false));
		TypedQuery<Account> query = em.createQuery(cq);
		return query.getResultList();
	}
